...
from .tmdb_service import get_popular_movies, get_now_playing_movies, get_movie_details, get_top_rated_movies, get_random_popular_movies, search_and_filter_movies, get_genres
...
def catalogo_peliculas(request):
    ...
    genres = get_genres() # Get all genres for the filter dropdown
    
    # Mapping visual filters to TMDb sort parameters
    sort_mapping = {
        'populares': 'popularity.desc',
        'valoradas': 'vote_average.desc',
        'cartelera': 'primary_release_date.desc',
    }
    sort_by = sort_mapping.get(filtro, 'popularity.desc')

    # Regla: si hay búsqueda o filtro por género, usamos la función unificada.
    # Si no, mantenemos la lógica anterior pero ahora:
    # - 'populares' => populares
    # - 'valoradas' => mejores valoradas
    # - 'cartelera' => novedades
    if query or genre_id:
        peliculas = search_and_filter_movies(query=query, genre_id=genre_id, sort_by=sort_by)
    else:
        peliculas = {
            'cartelera': get_now_playing_movies,        # novedades
            'valoradas': get_top_rated_movies,        # mejor valoradas
            'populares': get_popular_movies,          # populares
        }.get(filtro, get_popular_movies)()
        
    # Convert genre_id to int for template comparison if it exists
    current_genre_int = int(genre_id) if genre_id and genre_id.isdigit() else None
    ...
    return render(request, 'movies/peliculas.html', {
        ...
        'current_genre': current_genre_int, # Pass current genre ID for selection
        'search_query': query, # Pass search query to the template
    })
